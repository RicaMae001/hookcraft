...
<head>
    ...
    <style>
        ...
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .gcash-upload-section { background:rgba(99,179,237,0.1); border:2px dashed var(--info); padding:16px; border-radius:8px; margin-top:12px; }
        .payment-proof-preview { max-width:100%; max-height:240px; border-radius:8px; border:2px solid var(--info); cursor:pointer; }

        /* ── PAYMENT PROOF ON CARD ── */
        .card-payment-proof-section {
            display: flex;
            align-items: center;
            gap: 10px;
            background: var(--light-bg);
        ...
        .card-proof-status.missing  { color: var(--danger); }
        ...
        /* COD confirmation strip on card */
        .card-cod-strip {
            display: flex;
            align-items: center;
            gap: 8px;
        ...
        .card-cod-strip i { color: var(--warning); flex-shrink: 0; }

        /* Modal proof section — more prominent */
        .modal-proof-box {
        ...
        .modal-proof-img-wrap img:hover {
            transform: scale(1.02);
            box-shadow: 0 8px 24px rgba(99,179,237,0.3);
        }
        .modal-proof-no-upload {
            background: rgba(252,129,129,0.08);
        ...
        .modal-proof-no-upload i { display: block; font-size: 1.6rem; margin-bottom: 6px; opacity: 0.7; }
        ...
        .delivery-modal {
            ...
            animation: slideModal 0.28s cubic-bezier(0.34,1.3,0.64,1);
            position: relative;
        }
        ...
        @keyframes dashMove { from { stroke-dashoffset: 80; } to { stroke-dashoffset: 0; } }
        .leaflet-overlay-pane svg .animated-dash { animation: dashMove 1.4s linear infinite; }
        .leaflet-pane.leaflet-overlay-pane path.animated-dash {
        ...
        .gps-permission-banner i {
            color: var(--warning);
            font-size: 1.5rem;
        }
        .gps-permission-banner-content {
            flex: 1;
        }
        .gps-permission-banner-title {
            font-weight: 700;
            color: var(--text-primary);
            margin-bottom: 4px;
        }
        .gps-permission-banner-text {
            font-size: 0.85rem;
            color: var(--text-secondary);
            margin-bottom: 12px;
        }
...
<main class="main-content">
    ...
    <div class="row">
        ...
        <div class="col-lg-6 mb-4">
            ...
            {{-- ── CLICKABLE CARD ── --}}
            <div class="order-card" onclick="openDeliveryModal({{ $delivery->id }})">
                ...
                {{-- Mini map — shown for all statuses except Cancelled --}}
                @if($status !== 'Cancelled')
        ...
        {{-- ═══════════════════════════════════════════
             DELIVERY DETAIL MODAL #{{ $delivery->id }}
        ════════════════════════════════════════════ --}}
        <div class="delivery-modal-overlay" id="modal-overlay-{{ $delivery->id }}"
            ...
            <div class="delivery-modal" onclick="event.stopPropagation()">
                ...
                <div class="dmodal-body">
                    ...
                    {{-- Big map — shown for all statuses except Cancelled --}}
                    @if($status !== 'Cancelled')
                        ...
                        <div class="modal-map-header">
                            <div class="modal-map-title">
                                @if($status === 'Out for Delivery')
                                    ...
                                    Route to Customer
                                @elseif($status === 'Delivered')
                                    <i class="fas fa-check-circle me-1" style="color:#48BB78;"></i>
                                    Delivered Location
                                @else
                    ...
                    {{-- Turn-by-turn directions (only useful when Out for Delivery) --}}
                    @if($status === 'Out for Delivery')
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="text-center mb-3">
                    <i class="fas fa-map-marked-alt fa-3x" style="color:var(--warning);"></i>
                </div>
                <p class="mb-3">To track your delivery route and provide turn-by-turn directions, we need access to your location.</p>
                <div class="alert alert-info mb-3">
                    <strong><i class="fas fa-info-circle me-2"></i>How to enable:</strong>
                </div>
                <div class="mb-3">
                    ...
                    <div class="d-flex align-items-center mb-2">
                        ...
                        <span>Find <strong>Location</strong> permission and set it to <strong>Allow</strong></span>
                    </div>
                    <div class="d-flex align-items-center mb-2">
                        <span class="badge bg-primary rounded-circle me-2">3</span>
                        <span>Refresh the page and click "Allow" when prompted</span>
                    </div>
                </div>
...
function showGpsPermissionModal() {
    try {
        const modal = new bootstrap.Modal(document.getElementById('gpsPermissionModal'));
        modal.show();
    } catch (e) {
        console.log('GPS modal error:', e);
    }
}
...
function buildMap(id, destLat, destLng, mapType, deliveryStatus) {
    ...
    const pinPopup = isDelivered ? '✅ Delivered here' : '📦 Deliver here';

    L.marker([destLat, destLng], {
        icon: L.divIcon({ className: '', html: pinHtml, iconSize: [32,32], iconAnchor: [16,32] })
    }).addTo(map).bindPopup(`<b>${pinPopup}</b>`);

    const statusKey = isModal ? 'modal-map-status-' + id : 'mini-status-' + id;

    // Delivered / Pending — just show pinned location, route from store
    if (!isLive) {
        // Draw store → customer route for Pending/Delivered
        const storeToCustomerUrl = 'https://router.project-osrm.org/route/v1/driving/'
            + STORE_LNG + ',' + STORE_LAT + ';'
            + destLng   + ',' + destLat
            + '?overview=full&geometries=geojson';

        fetch(storeToCustomerUrl)
            .then(r => r.json())
            .then(data => {
                if (data.code === 'Ok' && data.routes?.length) {
                    ...
                    L.marker([STORE_LAT, STORE_LNG], {
                        icon: L.divIcon({
                            className: '',
                            html: '<div style="background:linear-gradient(135deg,#667eea,#764ba2);width:28px;height:28px;border-radius:50%;border:3px solid #fff;display:flex;align-items:center;justify-content:center;box-shadow:0 2px 8px rgba(102,126,234,0.5);"><i class="fas fa-store" style="color:#fff;font-size:0.65rem;"></i></div>',
                            iconSize: [28,28], iconAnchor: [14,14]
                        })
                    }).addTo(map).bindPopup('<b>🏪 Store (Hipodromo)</b>');

                    L.polyline(coords, {
                        color: isDelivered ? '#48BB78' : '#667eea',
                        weight: isModal ? 5 : 3,
                        opacity: 0.7,
                    ...
                    if (isModal) {
                        ...
                        setEl('modal-maps-link-' + id,
                            `<a href="https://www.google.com/maps/dir/${STORE_LAT},${STORE_LNG}/${destLat},${destLng}" target="_blank"
                                style="color:#fff;font-size:0.75rem;background:rgba(255,255,255,0.2);padding:3px 10px;border-radius:20px;text-decoration:none;">
                                <i class="fas fa-map-marked-alt me-1"></i>Open in Maps
                            </a>`, true);
                    } else {
    ...
    // ── Out for Delivery — Store → Customer route + live rider dot ──
    checkGpsPermission().then(permissionState => {
        ...
        if (permissionState === 'prompt' || permissionState === 'unsupported') {
            showGpsPromptBanner(id, mapType, destLat, destLng);
        }

        // Draw route from store → customer first (no GPS needed)
        drawRoute(map, destLat, destLng, id, mapType);

        // Then overlay live rider marker using GPS
...
function showGpsPromptBanner(id, mapType, destLat, destLng) {
    ...
    const stepsEl = document.getElementById('steps-list-' + id);
    if (!stepsEl) return;
    stepsEl.innerHTML = `
        <div class="gps-permission-banner">
            <i class="fas fa-map-marker-alt"></i>
            <div class="gps-permission-banner-content">
                <div class="gps-permission-banner-title">Location access requested</div>
                <div class="gps-permission-banner-text">
                    Allow location access to see your live position overlaid on the route.
                </div>
                <div class="gps-permission-banner-buttons">
                    <button class="gps-retry-btn" onclick="retryGpsPermission(${id}, ${destLat}, ${destLng})">
                        <i class="fas fa-sync-alt"></i>Retry GPS
                    </button>
                    <a href="https://www.google.com/maps/dir/${STORE_LAT},${STORE_LNG}/${destLat},${destLng}"
                       target="_blank" class="gps-manual-btn">
...
function showGpsBlockedBanner(id, mapType, destLat, destLng) {
    ...
    if (!stepsEl) return;
    stepsEl.innerHTML = `
        <div class="gps-permission-banner" style="background:rgba(252,129,129,0.15);border-color:var(--danger);">
            <i class="fas fa-exclamation-triangle" style="color:var(--danger);"></i>
            <div class="gps-permission-banner-content">
                <div class="gps-permission-banner-title">Location access is blocked</div>
                <div class="gps-permission-banner-text">
                    The route from store to customer is shown. To enable your live rider dot:
                ...
                <div class="gps-permission-banner-buttons">
                    <button class="gps-retry-btn" onclick="showGpsPermissionModal()">
                        <i class="fas fa-question-circle"></i>How to enable
                    </button>
                    <a href="https://www.google.com/maps/dir/${STORE_LAT},${STORE_LNG}/${destLat},${destLng}"
                       target="_blank" class="gps-manual-btn">
                        <i class="fas fa-map-marked-alt"></i>Open in Google Maps
...
function retryGpsPermission(id, destLat, destLng) {
    ...
    const stepsEl = document.getElementById('steps-list-' + id);
    if (stepsEl) stepsEl.innerHTML = '<div class="step-item"><span style="color:var(--text-secondary);">Requesting location access...</span></div>';
...
function requestGPS(map, id, destLat, destLng, mapType, statusKey, isModal) {
    ...
    navigator.geolocation.getCurrentPosition(
        pos => {
            ...
            const trackerKey = mapType + id;
            if (liveTrackers[trackerKey]) clearInterval(liveTrackers[trackerKey]);
            liveTrackers[trackerKey] = setInterval(() => {
                navigator.geolocation.getCurrentPosition(
...
// Rider markers keyed by delivery id
const riderMarkers = {};
        ...
        riderMarkers[id] = L.marker([rLat, rLng], {
            icon: L.divIcon({
                ...
                html: '<div style="position:relative;display:inline-block;">'
                    ...
                    + 'box-shadow:0 0 0 4px rgba(67,233,123,0.35),0 4px 14px rgba(67,233,123,0.5);'
                    + 'display:flex;align-items:center;justify-content:center;">'
                    + '<i class="fas fa-motorcycle" style="color:#fff;font-size:1.1rem;"></i>'
                    + '</div>'
                    + '<div style="position:absolute;top:-26px;left:50%;transform:translateX(-50%);'
                    + 'background:linear-gradient(135deg,#43e97b,#38f9d7);'
                    + 'color:#fff;font-size:9px;padding:2px 8px;border-radius:12px;'
...
// ── Main route drawer: ALWAYS Store → Customer ──────────────────────
async function drawRoute(map, destLat, destLng, id, mapType) {
    ...
    try {
        ...
        // Route lines
        L.polyline(coords, {
            color: '#667eea', weight: isModal ? 14 : 10, opacity: 0.18,
            lineJoin: 'round', lineCap: 'round'
        }).addTo(map);

        const routeLine = L.polyline(coords, {
            color: '#667eea', weight: isModal ? 6 : 4, opacity: 0.92,
            lineJoin: 'round', lineCap: 'round'
        }).addTo(map);

        L.polyline(coords, {
            color: '#fff', weight: isModal ? 2.5 : 1.5, opacity: 0.7,
        ...
        console.log('Routing error, fallback straight line:', e);
        const line = L.polyline([[STORE_LAT, STORE_LNG], [destLat, destLng]], {
            color: '#667eea', weight: 4, opacity: 0.75, dashArray: '9,9'
        }).addTo(map);
        ...
        if (isModal) {
            ...
            setEl('steps-list-' + id,
                '<div class="step-item"><span style="color:var(--text-secondary);font-size:0.82rem;">Road directions unavailable.</span></div>', true);
        } else {
            setEl('mini-dist-' + id, fmtDist(d));
        }
...
function buildProgressBar(id, pct) {
    ...
    if (existing) {
        ...
        const label = document.getElementById('progress-bar-label-' + id);
    ...
    bar.innerHTML = '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:5px;">'
        + '<span style="font-size:0.75rem;font-weight:700;color:var(--text-secondary);text-transform:uppercase;letter-spacing:0.05em;"><i class="fas fa-route me-1"></i>Delivery Progress</span>'
        + '<span id="progress-bar-label-'+id+'" style="font-size:0.75rem;font-weight:700;color:var(--primary-blue);">'+pct+'% complete</span>'
        + '</div>'
        + '<div style="background:var(--border-color);border-radius:99px;height:8px;overflow:hidden;">'
        + '<div id="progress-bar-fill-'+id+'" style="height:100%;width:'+pct+'%;background:linear-gradient(90deg,#667eea,#764ba2);border-radius:99px;transition:width 0.6s ease;box-shadow:0 0 8px rgba(102,126,234,0.5);"></div>'
        + '</div>'
        + '<div style="display:flex;justify-content:space-between;margin-top:3px;font-size:0.68rem;color:var(--text-secondary);"><span>🏪 Store</span><span>🏠 Customer</span></div>';
...
function buildTurnByTurn(route, id) {
    ...
    if (!steps.length) {
        container.innerHTML = '<div class="step-item"><span style="color:var(--text-secondary);font-size:0.82rem;">No steps available.</span></div>';
        return;
    }
    ...
    container.innerHTML = steps.map((s, i) => {
        ...
        const desc = type === 'arrive'
            ? '🏁 Arrive at destination'
            : `${cap(type.replace('-',' '))}${mod ? ' '+mod.replace('-',' ') : ''}${name ? ' on <b>'+name+'</b>' : ''}`;
        return `<div class="step-item">
            <div class="step-num">${i+1}</div>
            <div>
                <div class="step-text">${icon} ${desc}</div>
