    ...
    <div class="admin-chat-layout">
        ...
        <div class="admin-chat-sidebar" id="adminSidebar">
            ...
            <div class="sidebar-tabs">
                <button class="sidebar-tab active" id="tabOrders" onclick="switchTab('orders',this)">
                    <i class="fas fa-box me-1"></i> Orders
                </button>
                <button class="sidebar-tab" id="tabCustomize" onclick="switchTab('customizations',this)">
                    ...
                    <div class="messages-wrapper">
                        ...
                        @foreach($messages as $message)
                            @php
                                ...
                                $hasCustomize = !empty($message->customize_name);
                                $cSt          = $cStatusMap[$message->customize_status ?? 'Pending'] ?? $cStatusMap['Pending'];
                                // Parse materials JSON if present
                                $materials = [];
                                if (!empty($message->customize_materials)) {
                                    $materials = json_decode($message->customize_materials, true) ?: [];
                                }
                                $materialsTotal = array_sum(array_column($materials, 'subtotal'));
                            @endphp
                                            ...
                                            @if($hasCustomize)
                                                <div class="customize-card-bubble customer-customize">
                                                    ...
                                                    <img class="customize-bubble-img" src="{{ $message->customize_image }}" alt="Custom Image"
                                                         ...
                                                         onerror="this.style.display='none'">
                                                    @endif
                                                    @if($message->customize_details)
                                                        <div class="customize-bubble-row">📝 {{ $message->customize_details }}</div>
                                                    @endif
                                                    @if($message->customize_instructions)
                                                        <div class="customize-bubble-row">💬 <em>{{ $message->customize_instructions }}</em></div>
                                                    @endif
                                                    @if(count($materials) > 0)
                                                        <div class="customize-materials-block">
                                                            <div class="cmb-title">📋 Materials / Options</div>
                                                            <table class="cmb-table">
                                                                <thead><tr>
                                                                    <td>Item</td><td class="text-center">Qty</td><td>Unit</td><td class="text-right">Sub</td>
                                                                </tr></thead>
                                                                <tbody>
                                                                @foreach($materials as $mat)
                                                                    <tr>
                                                                        <td>{{ $mat['label'] ?? 'Item' }}</td>
                                                                        <td class="text-center">x{{ $mat['quantity'] ?? 1 }}</td>
                                                                        <td>₱{{ number_format($mat['unit_price'] ?? 0, 2) }}</td>
                                                                        <td class="text-right font-bold">₱{{ number_format($mat['subtotal'] ?? 0, 2) }}</td>
                                                                    </tr>
                                                                @endforeach
                                                                </tbody>
                                                            </table>
                                                            <div class="cmb-total">Customer Estimate: ₱{{ number_format($materialsTotal, 2) }}</div>
                                                        </div>
                                                    @endif
                                                    @if($message->customize_price)
                                                        <div class="customize-bubble-row font-bold">💰 ₱{{ number_format($message->customize_price,2) }}</div>
                                                    @endif
                                        ...
                                        <div class="message-bubble staff-bubble">
                                            ...
                                            @if($hasCustomize)
                                                <div class="customize-card-bubble staff-customize">
                                                    ...
                                                    @endif
                                                    @if(count($materials) > 0)
                                                        <div class="customize-materials-block staff-mat">
                                                            <div class="cmb-title">📋 Materials / Options</div>
                                                            <table class="cmb-table">
                                                                <thead><tr>
                                                                    <td>Item</td><td class="text-center">Qty</td><td>Unit</td><td class="text-right">Sub</td>
...
.sidebar-order-img {
    ...
    border: 2px solid var(--border-color); cursor: zoom-in; transition: transform 0.2s;
}
...
.sidebar-share-btn {
    ...
    background: linear-gradient(135deg, var(--primary-pink), var(--secondary));
    color: white; border: none; font-size: 0.72rem; font-weight: 600;
...
.sidebar-custom-mats    { margin-top: 5px; padding: 5px 7px; background: rgba(255,152,0,0.06); border-radius: 7px; border: 1px solid rgba(255,152,0,0.15); }
.sidebar-custom-mats-title { font-size: 0.67rem; font-weight: 700; color: #E65100; text-transform: uppercase; margin-bottom: 3px; }
.sidebar-custom-mat-row { font-size: 0.68rem; color: var(--text-secondary); display: flex; justify-content: space-between; padding: 1px 0; }
.sidebar-custom-mats-total { font-size: 0.72rem; font-weight: 700; color: #E65100; text-align: right; margin-top: 3px; border-top: 1px dashed rgba(255,152,0,0.3); padding-top: 3px; }
.sidebar-share-btn-outline {
    width: 100%; margin-top: 7px; padding: 5px; border-radius: 8px;
    background: transparent; border: 1.5px solid var(--primary-pink);
    color: var(--primary-pink); font-size: 0.72rem; font-weight: 600;
...
.chat-body {
    ...
    background: var(--light-bg); position: relative; padding: 1.5rem;
}
...
.message-bubble {
    ...
    border-radius: 14px; word-wrap: break-word; line-height: 1.5;
...
.customize-bubble-img    { width: 100%; max-height: 120px; object-fit: cover; border-radius: 8px; margin: 5px 0; cursor: zoom-in; display: block; transition: transform 0.2s; border: 1px solid rgba(255,255,255,0.25); }
.customize-bubble-img:hover { transform: scale(1.02); }
.customize-bubble-row    { font-size: 12px; margin: 4px 0; opacity: 0.9; }
...
/* Materials table inside bubble */
.customize-materials-block {
    margin: 7px 0; padding: 7px 9px; border-radius: 9px;
}
.customer-customize .customize-materials-block { background: rgba(255,152,0,0.08); border: 1px solid rgba(255,152,0,0.2); }
.staff-customize .customize-materials-block { background: rgba(255,255,255,0.12); border: 1px solid rgba(255,255,255,0.25); }
.cmb-title { font-size: 11px; font-weight: 700; margin-bottom: 5px; opacity: 0.85; }
.cmb-table { width: 100%; border-collapse: collapse; font-size: 11px; }
.cmb-table td { padding: 2px 3px; }
.cmb-table thead td { opacity: 0.6; font-size: 10px; }
.cmb-table .text-center { text-align: center; }
.cmb-table .text-right  { text-align: right; }
.customer-customize .cmb-table .font-bold { color: #E65100; }
.cmb-total { font-size: 11px; font-weight: 800; text-align: right; margin-top: 4px; padding-top: 4px; border-top: 1px dashed rgba(0,0,0,0.1); }
.customer-customize .cmb-total { color: #E65100; border-top-color: rgba(255,152,0,0.3); }
.staff-customize .cmb-total    { color: #FFD54F; border-top-color: rgba(255,255,255,0.25); }
.staff-mat .cmb-table .font-bold { color: #FFD54F; }
...
.img-modal-close  { background: none; border: none; font-size: 1.3rem; cursor: pointer; color: #6c757d; padding: 0 0.25rem; transition: color 0.15s; }
...
function buildMaterialsTable(materialsJson, isStaff) {
    if (!materialsJson) return '';
    let mats = [];
    try { mats = typeof materialsJson === 'string' ? JSON.parse(materialsJson) : materialsJson; } catch(e){ return ''; }
    if (!mats || !mats.length) return '';
    const side = isStaff ? 'staff-mat' : '';
    const rows = mats.map(m => `
        <tr>
            <td>${esc(m.label||'Item')}</td>
            <td class="text-center">x${m.quantity||1}</td>
            <td>₱${fmt(m.unit_price||0)}</td>
            <td class="text-right font-bold">₱${fmt(m.subtotal||0)}</td>
        </tr>`).join('');
    const total = mats.reduce((s,m) => s + (parseFloat(m.subtotal)||0), 0);
    return `<div class="customize-materials-block ${side}">
        <div class="cmb-title">📋 Materials / Options</div>
        <table class="cmb-table">
            <thead><tr>
                <td>Item</td><td class="text-center">Qty</td><td>Unit</td><td class="text-right">Sub</td>
        ...
        <div class="cmb-total">Customer Estimate: ₱${fmt(total)}</div>
    </div>`;
}
...
function buildProductCard(msg) {
    ...
    return `<div class="product-card-bubble">
        <img class="product-card-img" src="${esc(msg.product_image)}"
             alt="${esc(msg.product_name)}"
...
/* ── Build customize card ── */
function buildCustomizeCard(msg, isStaff) {
    if (!msg.customize_name && !msg.customize_image) return '';
    const side    = isStaff ? 'staff-customize' : 'customer-customize';
    const cStatus = msg.customize_status || 'Pending';
    const imgHtml = msg.customize_image
        ? `<img class="customize-bubble-img" src="${esc(msg.customize_image)}"
                ...
                onerror="this.style.display='none'">` : '';
    const detailHtml  = msg.customize_details    ? `<div class="customize-bubble-row">📝 ${esc(msg.customize_details)}</div>` : '';
    const instrHtml   = msg.customize_instructions ? `<div class="customize-bubble-row">💬 <em>${esc(msg.customize_instructions)}</em></div>` : '';
    const mats        = buildMaterialsTable(msg.customize_materials, isStaff);
    const priceHtml   = msg.customize_price      ? `<div class="customize-bubble-row font-bold">💰 &#8369;${fmt(msg.customize_price)}</div>` : '';
    return `<div class="customize-card-bubble ${side}">
        <div class="customize-bubble-header">
            <div class="customize-bubble-icon">🎨</div>
            <div>
        ...
        ${imgHtml}${detailHtml}${instrHtml}${mats}${priceHtml}
...
/* ── Send admin message ── */
async function sendAdminMessage(message, extraPayload) {
    if (!message) return;
    const payload = Object.assign({ session_id: SESSION.id, message }, extraPayload || {});
    try {
        ...
        if (data.success) {
            addMessage(message, 'staff', new Date().toISOString(), extraPayload ? Object.assign({message}, extraPayload) : null);
...
async function loadSidebarOrders() {
    try {
        ...
        data.orders.forEach(order => {
            const imgsHtml = (order.product_images || []).slice(0,4).map(i =>
                `<img class="sidebar-order-img" src="${esc(i.image_url)}" title="${esc(i.name)}"
                      onerror="this.style.display='none'"
                      onclick="openImageModal('${safe(i.image_url)}','${safe(i.name)}')">`
            ).join('');
            ...
            card.innerHTML = `
                ...
                <div class="sidebar-order-img-wrap">${imgsHtml}</div>
    ...
    try {
        ...
        data.customizations.forEach(c => {
            ...
            const imgHtml = c.image_url
                ? `<img class="sidebar-custom-img" src="${esc(c.image_url)}"
                        onerror="this.style.display='none'"
                        onclick="openImageModal('${safe(c.image_url)}','${safe(c.customization_name||'')}')">` : '';

            const priceHtml = c.admin_price
                ? `<div class="sidebar-custom-detail"><strong>Admin Price:</strong> &#8369;${fmt(c.admin_price)}</div>` : '';

            const detail = (c.customization_details||'').substring(0,70) + ((c.customization_details||'').length>70?'…':'');

            // Materials summary
            let matsHtml = '';
            if (c.options && c.options.length > 0) {
                const matRows = c.options.map(opt => {
                    const p = opt.parsed || {};
                    return `<div class="sidebar-custom-mat-row">
                        <span>${esc(p.label||opt.option_type||'Item')} x${p.quantity||1}</span>
                        <span>₱${fmt(p.subtotal||opt.additional_price||0)}</span>
                    </div>`;
                }).join('');
                matsHtml = `<div class="sidebar-custom-mats">
                    <div class="sidebar-custom-mats-title"><i class="fas fa-list-ul me-1"></i>Materials</div>
                    ${matRows}
                    <div class="sidebar-custom-mats-total">Est: &#8369;${fmt(c.options_total||0)}</div>
                </div>`;
            }
        ...
        document.getElementById('customsLoading').innerHTML = '<span style="color:red;font-size:0.8rem">Failed to load</span>';
    }
}
...
async function shareOrderInChat(order) {
    ...
    const firstItem = (order.product_images && order.product_images.length) ? order.product_images[0] : null;
    await sendAdminMessage(msg, firstItem ? {
        product_name: firstItem.name, product_price: firstItem.price, product_image: firstItem.image_url
    } : null);
}

async function shareCustomInChat(c) {
    const msg = `Customization ${c.ref} "${c.customization_name}" — Status: ${c.status}` +
        (c.admin_price ? ` | Admin Price: &#8369;${fmt(c.admin_price)}` : ' (awaiting pricing)');

    // Build materials JSON for sending
    ...
    await sendAdminMessage(msg, {
        customize_ref:          c.ref,
        customize_name:         c.customization_name,
        customize_details:      c.customization_details,
        customize_instructions: c.special_instructions,
        customize_status:       c.status,
        customize_price:        c.admin_price || c.total_price,
        customize_image:        c.image_url,
        customize_materials:    materialsJson,
    });
}
